...
const storedLeads: LeadData[] = [];

// Health check
app.get("/api/health", (_req, res) => {
  res.json({ status: "ok", app: "Fazenda Jutuba Server" });
});
...
app.post("/api/lead", (req, res) => {
  ...
  console.log(" Novo lead cadastrado na Fazenda Jutuba:", newLead);
...
// AI Chatbot endpoint via Gemini API
app.post("/api/chat", async (req, res) => {
  ...
  // Fallback system response if Gemini key is missing
...
Idiomas: Sempre responda em Português do Brasil com excelente redação. Se o usuário perguntar algo fora do contexto do imóvel, responda gentilmente e redirecione para a beleza e oportunidade da Fazenda Jutuba.`;
    ...
    // Format historical messages for chat context if provided
    ...
    console.error("Erro na API Gemini no servidor:", error);
    const fallbackReply = getFallbackResponse(message);
    return res.json({
      reply: fallbackReply,
...
// Helper for rule-based fallbacks
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(` Servidor Fazenda Jutuba ativo em http://localhost:${PORT}`);
  });
}
